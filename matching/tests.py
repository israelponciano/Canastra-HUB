...
class CandidatosParaVagaViewTest(TestCase):
    ...
    def test_retorna_200_com_candidatos(self):
        resultados = [
    ...
    def test_retorna_400_com_top_invalido(self):
        with patch('matching.views.get_matcher', return_value=MagicMock()):
            response = self.client.get('/matching/candidatos/1/?top=abc')
        self.assertEqual(response.status_code, 400)

    def test_retorna_400_com_top_acima_do_limite(self):
        with patch('matching.views.get_matcher', return_value=MagicMock()):
            response = self.client.get('/matching/candidatos/1/?top=51')
        self.assertEqual(response.status_code, 400)

    def test_repassa_top_ao_matcher(self):
        mock_matcher = MagicMock()
    ...
    def test_retorna_500_em_erro_interno(self):
        mock_matcher = MagicMock()
        mock_matcher.match_resumes_for_job.side_effect = RuntimeError("crash inesperado")

        with patch('matching.views.get_matcher', return_value=mock_matcher):
            response = self.client.get('/matching/candidatos/1/')
...
class VagasParaUsuarioViewTest(TestCase):
    ...
    def test_retorna_200_com_vagas(self):
        resultados = [
    ...
    def test_repassa_min_score_ao_matcher(self):
        mock_matcher = MagicMock()
    ...
    def test_retorna_400_com_top_zero(self):
        with patch('matching.views.get_matcher', return_value=MagicMock()):
            response = self.client.get('/matching/vagas-para/7/?top=0')
        self.assertEqual(response.status_code, 400)
